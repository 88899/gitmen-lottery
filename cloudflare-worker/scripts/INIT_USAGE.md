# init.sh 使用指南

## 概述

`init.sh` 脚本用于批量导入历史数据，支持双色球、大乐透和七星彩。

## 更新内容 ✅

- ✅ 支持双色球（ssq）
- ✅ 支持大乐透（dlt）
- ✅ 支持所有类型（不带参数）
- ✅ 通过参数指定彩票类型
- ✅ 自动调用对应的 API 接口
- ✅ 显示彩票类型和名称

## 使用方法

### 基本用法

```bash
# 初始化所有类型（推荐）
./scripts/init.sh

# 初始化指定类型
./scripts/init.sh ssq    # 仅双色球
./scripts/init.sh dlt    # 仅大乐透
```

### 配置文件

在 `.env` 文件中配置：

```bash
# Worker URL
WORKER_URL=https://your-worker.workers.dev

# API Key
API_KEY=your-api-key

# 爬取间隔（秒）
SLEEP_TIME=120

# 每日请求限制
DAILY_REQUEST_LIMIT=500

# 是否自动跨天继续
AUTO_CONTINUE=false

# 代理配置（可选）
USE_PROXY=false
PROXY_PORT=7890
```

## 功能特性

### 1. 支持多彩票类型

```bash
# 双色球
./scripts/init.sh ssq

# 大乐透
./scripts/init.sh dlt
```

### 2. 自动断点续传

- 自动跳过已存在的数据
- 支持中断后继续执行
- 记录每日请求次数

### 3. 每日请求限制

- 默认每日限制 500 次
- 达到限制后自动停止或等待跨天
- 可配置 `AUTO_CONTINUE=true` 自动跨天继续

### 4. 智能提示

- 显示当前进度
- 提示数据量接近完成
- 显示数据源和查询参数

## 示例

### 示例1：初始化双色球

```bash
$ ./scripts/init.sh ssq

🚀 双色球数据初始化
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
彩票类型: ssq (双色球)
Worker URL: https://your-worker.workers.dev
每日限制: 500 次
爬取间隔: 120 秒
自动跨天: 否

📊 今日已执行: 0/500 次

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📊 执行第 1 次...
时间: 2025-11-18 13:00:00

{
  "success": true,
  "message": "2025 年数据爬取完成",
  "inserted": 131,
  "skipped": 0,
  "total": 3378,
  "year": 2025,
  "hasMore": false,
  "lotteryType": "ssq"
}

✅ 本批次成功
   新增: 131 条
   跳过: 0 条
   当前总计: 3378 条
   数据源: 500.com
   查询参数: start=25001, end=25200

✅ 本批次新增 131 条数据，继续爬取...

⏳ 等待 120 秒后继续...
   本次会话: 第 1 次 | 今日总计: 1/500 次
   （按 Ctrl+C 可随时停止）
```

### 示例2：初始化大乐透

```bash
$ ./scripts/init.sh dlt

🚀 大乐透数据初始化
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
彩票类型: dlt (大乐透)
Worker URL: https://your-worker.workers.dev
每日限制: 500 次
爬取间隔: 120 秒
自动跨天: 否

📊 今日已执行: 0/500 次

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📊 执行第 1 次...
时间: 2025-11-18 13:00:00

{
  "success": true,
  "message": "2025 年数据爬取完成",
  "inserted": 131,
  "skipped": 0,
  "total": 2799,
  "year": 2025,
  "hasMore": false,
  "lotteryType": "dlt"
}

✅ 本批次成功
   新增: 131 条
   跳过: 0 条
   当前总计: 2799 条
   数据源: 500.com
   查询参数: start=25001, end=25200

💡 提示：数据库已有 2799 条数据（大乐透从2007年开始，约有2700+期）
   如果数据量接近历史总期数，可能即将完成

✅ 本批次新增 131 条数据，继续爬取...

⏳ 等待 120 秒后继续...
   本次会话: 第 1 次 | 今日总计: 1/500 次
   （按 Ctrl+C 可随时停止）
```

### 示例3：数据爬取完成

```bash
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🎉 数据爬取完成！
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

📊 最终统计：
   总执行次数: 18 次
   数据库总数据量: 2799 条

📈 详细统计：
{
  "lottery_type": "dlt",
  "lottery_name": "大乐透",
  "total_count": 2799,
  "top_front_balls": [
    { "ball": "29", "count": 483 },
    { "ball": "35", "count": 466 }
  ],
  "top_back_balls": [
    { "ball": "10", "count": 503 },
    { "ball": "07", "count": 495 }
  ]
}

✅ 大乐透所有历史数据已成功导入！
```

## 工作流程

1. **读取配置**
   - 从 `.env` 文件读取配置
   - 验证 Worker URL 和 API Key

2. **验证彩票类型**
   - 检查参数是否为 `ssq`、`dlt`、`qxc` 或 `qlc`
   - 设置对应的彩票名称

3. **循环爬取**
   - 调用 `/init/{type}` API
   - 每次爬取一年的数据
   - 自动跳过已存在的数据

4. **智能停止**
   - 连续3次无新数据时停止
   - 达到每日限制时停止
   - 可配置自动跨天继续

5. **显示统计**
   - 显示爬取进度
   - 显示数据源信息
   - 显示最终统计

## 配置说明

### SLEEP_TIME（爬取间隔）

```bash
SLEEP_TIME=120  # 每次爬取间隔 120 秒
```

**建议值**：
- 快速模式：60 秒
- 正常模式：120 秒（推荐）
- 保守模式：180 秒

### DAILY_REQUEST_LIMIT（每日限制）

```bash
DAILY_REQUEST_LIMIT=500  # 每日最多 500 次
```

**说明**：
- 双色球：约 18 年数据，需要 18 次
- 大乐透：约 18 年数据，需要 18 次
- 建议设置为 500，足够完成初始化

### AUTO_CONTINUE（自动跨天）

```bash
AUTO_CONTINUE=false  # 不自动跨天
AUTO_CONTINUE=true   # 自动跨天继续
```

**说明**：
- `false`：达到每日限制后停止，需要手动重新运行
- `true`：达到每日限制后等待到第二天 00:00 自动继续

## 故障排查

### 问题1：认证失败

```
❌ 认证失败！请检查 .env 文件中的 API_KEY 是否正确
```

**解决**：
```bash
# 检查 .env 文件
cat .env | grep API_KEY

# 更新 API_KEY
vim .env
```

### 问题2：Worker URL 未配置

```
❌ 错误：请先配置 WORKER_URL
```

**解决**：
```bash
# 编辑 .env 文件
vim .env

# 设置 WORKER_URL
WORKER_URL=https://your-worker.workers.dev
```

### 问题3：不支持的彩票类型

```
❌ 错误：不支持的彩票类型 'xxx'
```

**解决**：
```bash
# 使用正确的彩票类型
./scripts/init.sh ssq  # 双色球
./scripts/init.sh dlt  # 大乐透
```

### 问题4：达到每日限制

```
⏸️  已达到今日请求限制
```

**解决方案1**：等待第二天重新运行
```bash
# 第二天继续
./scripts/init.sh dlt
```

**解决方案2**：启用自动跨天
```bash
# 编辑 .env
AUTO_CONTINUE=true

# 重新运行
./scripts/init.sh dlt
```

**解决方案3**：调整每日限制
```bash
# 编辑 .env
DAILY_REQUEST_LIMIT=1000

# 重新运行
./scripts/init.sh dlt
```

## 最佳实践

### 1. 首次初始化

```bash
# 1. 配置 .env
cp .env.example .env
vim .env

# 2. 初始化双色球
./scripts/init.sh ssq

# 3. 等待完成后，初始化大乐透
./scripts/init.sh dlt
```

### 2. 使用 screen 或 tmux

```bash
# 使用 screen（推荐）
screen -S lottery-init
./scripts/init.sh dlt
# 按 Ctrl+A, D 分离会话

# 重新连接
screen -r lottery-init

# 或使用 tmux
tmux new -s lottery-init
./scripts/init.sh dlt
# 按 Ctrl+B, D 分离会话

# 重新连接
tmux attach -t lottery-init
```

### 3. 后台运行

```bash
# 使用 nohup
nohup ./scripts/init.sh dlt > init-dlt.log 2>&1 &

# 查看日志
tail -f init-dlt.log

# 查看进程
ps aux | grep init.sh
```

### 4. 定时检查

```bash
# 查看今日执行次数
cat /tmp/lottery_init_count_$(date '+%Y-%m-%d').txt

# 查看数据库统计
curl https://your-worker.workers.dev/stats/dlt | jq
```

## 相关脚本

- `init.sh` - 批量初始化脚本（本脚本）
- `test-incremental.sh` - 测试增量爬取
- `diagnose.sh` - 诊断工具
- `set-strategies.sh` - 设置预测策略

## 相关文档

- [API 使用文档](../API_USAGE.md)
- [Worker 更新指南](../WORKER_DLT_UPDATE_GUIDE.md)
- [部署指南](../docs/DEPLOY.md)

---

**版本**：3.0.0  
**更新日期**：2025-11-18  
**说明**：支持多彩票类型，使用统一的增量爬取逻辑
