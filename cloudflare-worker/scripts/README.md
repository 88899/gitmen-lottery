# Cloudflare Worker 脚本说明

## 📋 脚本列表

### 1. init.sh ✅（已更新）

**功能**：批量导入历史数据

**支持**：
- ✅ 双色球（ssq）
- ✅ 大乐透（dlt）
- ✅ 所有类型（不带参数）

**使用**：
```bash
# 所有类型（推荐）
./scripts/init.sh

# 指定类型
./scripts/init.sh ssq    # 仅双色球
./scripts/init.sh dlt    # 仅大乐透
```

**文档**：[INIT_USAGE.md](./INIT_USAGE.md)

### 2. test-incremental.sh ⏳（待更新）

**功能**：测试增量爬取

**当前状态**：仅支持双色球

**需要更新**：添加彩票类型参数

**建议用法**：
```bash
# 双色球
./scripts/test-incremental.sh ssq

# 大乐透
./scripts/test-incremental.sh dlt
```

### 3. diagnose.sh ⏳（待更新）

**功能**：诊断工具

**当前状态**：仅支持双色球

**需要更新**：添加彩票类型参数

**建议用法**：
```bash
# 双色球
./scripts/diagnose.sh ssq

# 大乐透
./scripts/diagnose.sh dlt
```

### 4. set-strategies.sh ⏳（待更新）

**功能**：设置预测策略

**当前状态**：全局配置

**需要更新**：支持按彩票类型配置

**建议用法**：
```bash
# 双色球
./scripts/set-strategies.sh ssq frequency,balanced,coldHot 15

# 大乐透
./scripts/set-strategies.sh dlt frequency,coldHot 10
```

## 🔧 更新计划

### 优先级1：init.sh ✅
- [x] 支持彩票类型参数
- [x] 更新 API 调用路径
- [x] 更新提示信息
- [x] 创建使用文档

### 优先级2：test-incremental.sh ⏳
- [ ] 添加彩票类型参数
- [ ] 更新 API 调用路径
- [ ] 测试双色球和大乐透

### 优先级3：diagnose.sh ⏳
- [ ] 添加彩票类型参数
- [ ] 更新诊断逻辑
- [ ] 支持多彩票类型检查

### 优先级4：set-strategies.sh ⏳
- [ ] 支持按彩票类型配置
- [ ] 更新配置逻辑
- [ ] 创建使用文档

## 📖 使用指南

### 快速开始

```bash
# 1. 配置环境
cp ../.env.example ../.env
vim ../.env

# 2. 初始化所有类型（推荐）
./init.sh

# 或者分别初始化
./init.sh ssq    # 仅双色球
./init.sh dlt    # 仅大乐透
```

### 配置文件

在 `../.env` 文件中配置：

```bash
# Worker URL
WORKER_URL=https://your-worker.workers.dev

# API Key
API_KEY=your-api-key

# 爬取间隔（秒）
SLEEP_TIME=120

# 每日请求限制
DAILY_REQUEST_LIMIT=500

# 是否自动跨天继续
AUTO_CONTINUE=false
```

## 📚 文档

- [init.sh 使用指南](./INIT_USAGE.md) - 详细的使用说明
- [API 使用文档](../API_USAGE.md) - API 接口说明
- [Worker 更新指南](../WORKER_DLT_UPDATE_GUIDE.md) - Worker 更新步骤

## 🎯 下一步

### 立即可用
- ✅ `init.sh` - 已更新，可以使用

### 待更新
- ⏳ `test-incremental.sh` - 需要添加彩票类型参数
- ⏳ `diagnose.sh` - 需要添加彩票类型参数
- ⏳ `set-strategies.sh` - 需要支持按类型配置

### 建议
1. 先使用 `init.sh` 初始化数据
2. 等待其他脚本更新后再使用
3. 或手动调用 API 接口进行测试

## 🔗 相关链接

- [Cloudflare Worker 文档](https://developers.cloudflare.com/workers/)
- [D1 数据库文档](https://developers.cloudflare.com/d1/)
- [KV 存储文档](https://developers.cloudflare.com/kv/)

---

**版本**：2.0.0  
**更新日期**：2025-11-18  
**状态**：init.sh 已更新，其他脚本待更新
